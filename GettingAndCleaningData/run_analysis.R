...
dtAll <- data.table(melt(dtAll, id=c("subject", "label", "activityName"), variable.name = "featureCode"))
dtAll <- merge(dtAll, dtFeatures[, list(featureNum, featureCode, featureName)], by = "featureCode", all.x = TRUE)
